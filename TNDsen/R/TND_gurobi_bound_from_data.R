...
TND_gurobi_bounds_from_data = function(data, delta, gamma, xi, alpha, conf.type,...)
{

  random = !missing(alpha)
  multinomial_fit = fit_multinomial(data)
  ...
  for(i in 1:1)
  {
    ...
    Sigma_delta = kronecker(Sigma_o[1:3,], design_matrix[i,])  # 3d x 4
    ...
    #print(c(result$lower.bound, result$upper.bound))
